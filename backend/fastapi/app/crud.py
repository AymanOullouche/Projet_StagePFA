...
def create_etablissement(session: Session, data: schemas.EtablissementCreate) -> models.Etablissement:
    etab = models.Etablissement(**data.dict())
    session.add(etab)
    session.commit()
...
def create_inspection(session: Session, data: schemas.InspectionCreate) -> models.Inspection:
    inspection = models.Inspection(**data.dict())
...
def create_rapport(session: Session, data: schemas.RapportCreate) -> models.Rapport:
    rapport = models.Rapport(**data.dict())
    session.add(rapport)
    session.commit()
...
def create_inspection_image(session: Session, inspection_id: int, filename: str, content_type: str) -> models.InspectionImage:
    image = models.InspectionImage(inspection_id=inspection_id, filename=filename, content_type=content_type)
    session.add(image)
    session.commit()
